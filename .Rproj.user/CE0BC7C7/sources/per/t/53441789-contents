...
cvdData <- fread('data/mapResultsHighestRateCities.csv',  select=c('Local Authority', 'LLD_SD_ActualCases_wk46', 'LLD_SD_ActualMortalities_wk46', 'LLD_SD_ForecastCases_wk51', 'LLD_SD_ForecastMortalities_wk51', 'FLD_ForecastCases_wk51', 'FLD_ForecastMortality_wk51', 'LLD_SD_ForecastCases_wk51_Retail50Pct', 'LLD_SD_ForecastCases_wk51_FoodAccom50Pct', 'LLD_SD_ForecastCases_wk51_pub50Pct', 'LLD_SD_ForecastCases_wk51_quarantine100Pct', 'LLD_SD_ForecastCases_wk51_SchoolOpen50Pct', 'LLD_SD_ForecastCases_wk51_IntTravel50Pct' ), data.table=FALSE)
                          ...
                          # tabPanel("General Metrics", br(),
                                   sidebarLayout(
                                     sidebarPanel(style = "overflow-y:scroll; max-height: 600px; position:relative;",
									   ...
									   uiOutput("sliderHealthDeprivation"),
									   uiOutput("sliderLivingEnvironmentDeprivation"),
									   uiOutput("sliderPopulationEstimate"),
								  ...
								  mainPanel( 
									 tabsetPanel(id="tabs1",
									    ...
									    tabPanel("Map: Cases", br(),
										   ...
										   div(leafletOutput(outputId='map', width = "80em", height = "80em"), div(textOutput(outputId = 'summary'))), br(), br(),
										   div(leafletOutput(outputId='mapSim', width = "80em", height = "80em")),
										   hidden(div(id="MapButtonActualCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "mapActualProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Actual Cases Map</span>"), width= 120, style='padding:4px; font-size:80%')) ), br(), br(),
										   div(leafletOutput(outputId='mapActual', width = "80em", height = "80em")), br(), br()
									    ...
									    tabPanel("Map: Mortality", br(),
										   ...
										   div(leafletOutput(outputId='mapMtly', width = "80em", height = "80em")), br(), br(),
										   div(leafletOutput(outputId='mapSimMtly', width = "80em", height = "80em")),
										   
										   hidden(div(id="MapActualMtlyButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "mapActualMtlyProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Actual Mortality Map</span>"), width= 130, style='padding:4px; font-size:80%')) ), br(), br(),
										   div(leafletOutput(outputId='mapActualMtly', width = "80em", height = "80em")), br(), br()
										   
										),
									    tabPanel("Model", br(),
										   hidden(div(id="ModelButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "modelProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Variable Importance</span>"), width= 150, style='padding:4px; font-size:80%')) ), br(), br(),
										   plotlyOutput("histogrmVariableImprtnc", width = "50%", height = "50%"), br(), br(),
										   #uiOutput("Instrctn"), br(),
										   div(id="ModelCorrButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "modelCorrProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Variable Correlation</span>"), width= 150, style='padding:4px; font-size:80%')),
										   plotOutput(outputId = "histogrmVariableCorr", height = "600px"),
										   hidden(div(id="ModelCasesButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "modelCasesProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Cases Variable Importance</span>"), width= 170, style='padding:4px; font-size:80%')) ), br(), br(),
										   plotlyOutput("histogrmVariableCasesImprtnc", width = "50%", height = "50%"), br(), br(),
										   hidden(div(id="ModelMtlyButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "modelMtlyProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Mtly Variable Importance</span>"), width= 170, style='padding:4px; font-size:80%')) ), br(), br(),
										   plotlyOutput("histogrmVariableMtlyImprtnc", width = "50%", height = "50%"), br(), br()
										   
									    ),
									    tabPanel("Hotspots", br(),
										   hidden(div(id="ModelButtonHotSptCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "hotSptCasesProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Hotspots</span>"), width= 150, style='padding:4px; font-size:80%')) ), br(), br(),
										   plotlyOutput("effectsHotSptCases", width = "50%", height = "50%"), br(), br(),
										   plotlyOutput("effectsHotSptMtly", width = "50%", height = "50%"), br(), br(),
										   plotlyOutput("effectsHotSptPolicyAddtn", width = "30em", height = "30em"), br(), br()
...
#observe( { 
  ...
  output$locAuthCheckbx <- renderUI({div(style = "width: 1px;", checkboxGroupInput(inputId = "check2", label = "Choose", choices = sort(locAuthList), selected = NULL)) 
	  })
  ...
  observeEvent(input$tabs1, {
   if (isolate(input$tabs1) == "Trend") { 
	...
	show("metricsText")
	hide("ModelButtonCases")
	hide("ModelCasesButtonCases")
	hide("ModelMtlyButtonCases")
	hide("MapButtonActualCases")
	hide("MapActualMtlyButtonCases")
	hide("ModelButtonHotSptCases")
	...
	hide("ModelMtlyButtonCases")
	hide("MapButtonActualCases")
	show("MapActualMtlyButtonCases")
	hide("ModelButtonHotSptCases")

	
  } else if (isolate(input$tabs1) == "Model") { 
	
	show("ModelButtonCases")
	hide("MapButtonCases")
	hide("MapMtlyButtonCases")
	hide("TrendButtonCases")
	show("ResetPrmtrButton")
	show("processPlotText")
	show("metricsText")
	show("ModelCorrButtonCases")
	show("ModelCasesButtonCases")
	show("ModelMtlyButtonCases")
	hide("MapButtonActualCases")
	hide("MapActualMtlyButtonCases")
	hide("ModelButtonHotSptCases")

	
  }  else if (isolate(input$tabs1) == "Hotspots") { 
  ...
  observeEvent(input$LockDown, { 
		...
		if (input$LockDown == 1) {
				...
				updateSliderInput(session, "healthDeprivation", value=0)
				
				updateSliderInput(session, "livingEnvironmentDeprivation", value=0)
				
				updateSliderInput(session, "populationEstimate", value=0)
  ...
  output$sliderAccmNFoodSize <- renderUI({
			...
			if((flagResetLocalVar$reset_localVar  == TRUE | flagUpdtVarByLA$updateParamByLA == TRUE) & !input$nationalSetting) {
				...
				sliderInput(inputId = "AccmNFoodSize", label = "Number of Accomodation and Food services", 0, max(jointWeekNCases_Mltd_time$Accommodation_food_services, na.rm = TRUE), value = input$AccmNFoodSize, round = 2, animate = TRUE) %>% helper(type = "inline", title = "Number of Accomodation and Food services",
                                         content = c("The number of Accomodation and Food services (VAT and/or PAYE based) local units in the selected local authority.",
										 ""
										 ))
  ...
  output$sliderHealthDeprivation <- renderUI({
			
			
			if((flagResetLocalVar$reset_localVar  == TRUE | flagUpdtVarByLA$updateParamByLA == TRUE) & !input$nationalSetting) {
			
				 sliderInput(inputId = "healthDeprivation", label = "health and disability deprivation decile", min(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Decile, na.rm = TRUE), max(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Decile, na.rm = TRUE), value = mean(unique(jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$wk_int == max(jointWeekNCases_Mltd_time$wk_int) & jointWeekNCases_Mltd_time$lad19_nm %in% input$check2,]$Health_Deprivation_Disability_Decile)), round = 2, animate = TRUE) %>% helper(type = "inline",  title = "health and disability deprivation decile", content = c("measures the risk of premature death and the
					impairment of quality of life through poor physical or mental health. The domain measures morbidity,
					disability and premature mortality but not aspects of behaviour or environment that may be
					predictive of future health deprivation. Decile 1 represents the most deprived 10% (or decile) of neighbourhoods in England and Decile 10 represents the least deprived 10% (or decile) of neighbourhoods in England." ))
  
  
  
			} else if (input$nationalSetting) {
			
				sliderInput(inputId = "healthDeprivation", label = "health and disability deprivation decile", min(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Decile, na.rm = TRUE), max(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Decile, na.rm = TRUE), value = input$Health_Deprivation_Disability_Decile, round = 2, animate = TRUE) %>% helper(type = "inline",  title = "health and disability deprivation decile", content = c("measures the risk of premature death and the impairment of quality of life through poor physical or mental health. The domain measures morbidity,	disability and premature mortality but not aspects of behaviour or environment that may be
					predictive of future health deprivation. Decile 1 represents the most deprived 10% (or decile) of neighbourhoods in England and Decile 10 represents the least deprived 10% (or decile) of neighbourhoods in England." ))
			
			
			} else {
			
				sliderInput(inputId = "healthDeprivation", label = "health and disability deprivation decile", min(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Decile, na.rm = TRUE), max(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Decile, na.rm = TRUE), value = mean(unique(jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$wk_int == max(jointWeekNCases_Mltd_time$wk_int) & jointWeekNCases_Mltd_time$lad19_nm %in% input$check2,]$Health_Deprivation_Disability_Decile)), round = 2, animate = TRUE)  %>% helper(type = "inline",  title = "health and disability deprivation decile", content = c("Decile 1 represents the most deprived 10% (or decile) of neighbourhoods in England and Decile 10 represents the least deprived 10% (or decile) of neighbourhoods in England." )) %>% helper(type = "inline",  title = "health and disability deprivation decile", content = c("measures the risk of premature death and the impairment of quality of life through poor physical or mental health. The domain measures morbidity, disability and premature mortality but not aspects of behaviour or environment that may be
					predictive of future health deprivation. Decile 1 represents the most deprived 10% (or decile) of neighbourhoods in England and Decile 10 represents the least deprived 10% (or decile) of neighbourhoods in England." ))
  				
			}
  })
  
  output$sliderLivingEnvironmentDeprivation <- renderUI({
			
			
			if((flagResetLocalVar$reset_localVar  == TRUE | flagUpdtVarByLA$updateParamByLA == TRUE) & !input$nationalSetting) {
			
				 sliderInput(inputId = "livingEnvironmentDeprivation", label = "living environment deprivation decile", min(jointWeekNCases_Mltd_time$Living_Environment_Decile, na.rm = TRUE), max(jointWeekNCases_Mltd_time$Living_Environment_Decile, na.rm = TRUE), value = mean(unique(jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$wk_int == max(jointWeekNCases_Mltd_time$wk_int) & jointWeekNCases_Mltd_time$lad19_nm %in% input$check2,]$Living_Environment_Decile)), round = 2, animate = TRUE) %>% helper(type = "inline",  title = "living environment deprivation decile", content = c("measures the quality of the local environment. The indicators fall into two sub-domains. The ‘indoors’ living environment measures the quality of housing; while the ‘outdoors’ living environment contains measures of air quality. Decile 1 represents the most deprived 10% (or decile) of neighbourhoods in England and Decile 10 represents the least deprived 10% (or decile) of neighbourhoods in England." ))
  
			} else if (input$nationalSetting) {
			
				sliderInput(inputId = "livingEnvironmentDeprivation", label = "living environment deprivation decile", min(jointWeekNCases_Mltd_time$Living_Environment_Decile, na.rm = TRUE), max(jointWeekNCases_Mltd_time$Living_Environment_Decile, na.rm = TRUE), value = input$Living_Environment_Decile, round = 2, animate = TRUE) %>% helper(type = "inline",  title = "living environment deprivation decile", content = c("measures the quality of the local environment. The indicators fall into two sub-domains. The ‘indoors’ living environment measures the quality of housing; while the ‘outdoors’ living environment contains measures of air quality. Decile 1 represents the most deprived 10% (or decile) of neighbourhoods in England and Decile 10 represents the least deprived 10% (or decile) of neighbourhoods in England." ))
  ...
  output$sliderPopulationEstimate <- renderUI({
			
			
			if((flagResetLocalVar$reset_localVar  == TRUE | flagUpdtVarByLA$updateParamByLA == TRUE) & !input$nationalSetting) {
			
				 sliderInput(inputId = "populationEstimate", label = "2019 Population estimate", min(jointWeekNCases_Mltd_time$`Estimated Population  mid-2019`, na.rm = TRUE), max(jointWeekNCases_Mltd_time$`Estimated Population  mid-2019`, na.rm = TRUE), value = mean(unique(jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$wk_int == max(jointWeekNCases_Mltd_time$wk_int) & jointWeekNCases_Mltd_time$lad19_nm %in% input$check2,]$`Estimated Population  mid-2019`)), round = 2, animate = TRUE) 
  
			} else if (input$nationalSetting) {
			
				sliderInput(inputId = "populationEstimate", label = "2019 Population estimate", min(jointWeekNCases_Mltd_time$`Estimated Population  mid-2019`, na.rm = TRUE), max(jointWeekNCases_Mltd_time$`Estimated Population  mid-2019`, na.rm = TRUE), value = input$`Estimated Population  mid-2019`, round = 2, animate = TRUE) 
  ...
  observeEvent(input$loadModelButtonId, {
      ...
      loadModelVar$loadMdl <<- !loadModelVar$loadMdl 
				...
				samplesSim <- array(0, dim = c(length(subsetDt_indiv$week) - 1,
												2,
												dim(data_indiv)[[-1]]))  
				...
				for (i in (seq_along(1: (length(subsetDt_indiv$week) - 1))) ) {  # start from wk 5 to 41 (length 37) - 2
					...
					samplesSim[i,,] <- as.matrix(data_indiv[indices,]) 
				...
				transportSrvcIndx <- grep("Transport_Storage_inc_postal", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% wkRange,,transportSrvcIndx] <- scale(input$TransprtStrgSize, center = meanTrain[["Transport_Storage_inc_postal"]], scale = stdTrain[["Transport_Storage_inc_postal"]])
				...
				healthDeprivatnIndx <- grep("Health_Deprivation_Disability_Decile", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% wkRange,,healthDeprivatnIndx] <- scale(input$healthDeprivation, center = meanTrain[["Health_Deprivation_Disability_Decile"]], scale = stdTrain[["Health_Deprivation_Disability_Decile"]])
  ...
  #hot spot effect plot 
  observeEvent(input$hotSptCasesProcessId, {

  if (isolate(input$tabs1) == "Hotspots") { 
 
   
	plotDataHotspots <- eventReactive(input$hotSptCasesProcessId, {
				
		par(mar=c(10,4.5,7,7))
			

		p <- ggplot(cvdData, aes(`Local Authority`)) + theme_bw() + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ActualCases_wk46, linetype = "Number of Actual Cases for wk 46: Full lockdown", group = 1)) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastCases_wk51, linetype = "Cases Forecast for wk 51: LD_SD", group = 1)) + geom_line(size = 0.5, na.rm=TRUE, aes(y = FLD_ForecastCases_wk51, linetype = "Cases Forecast for wk 51: Full lockdown", group = 1)) + scale_linetype_manual(values = c('dotted', 'dashed', 'solid')) + labs(color='Policy Id') + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + xlab("Week") + ylab("Count") + ggtitle("Effect: cases by policy") + theme(plot.title = element_text(hjust = 0.5)) + scale_colour_manual(values = colorSet2) + guides(color = guide_legend(override.aes = list(size = 1.5)))
				...
				layout( legend=list(orientation = "h", x = 0.12, y = -0.99), xaxis=list(tickangle=270, title = "\n Location (Local Authority)", tickfont = list(size = '15')))
	...
	if (isolate(input$tabs1) == "Hotspots") {
		plotDataImpCases <<- isolate(plotDataHotspots)
	
	
		output$effectsHotSptCases <<- renderPlotly({
			plotDataImpCases()
		})
		
		outputOptions(output, "effectsHotSptCases", suspendWhenHidden=FALSE)
	...
	plotDataMtlyHotspots <- eventReactive(input$hotSptCasesProcessId, {
				
		par(mar=c(10,4.5,7,7))
			

		p <- ggplot(cvdData, aes(`Local Authority`)) + theme_bw() + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ActualMortalities_wk46, linetype = "Number of Actual mortalities for wk 46: Full lockdown", group = 1)) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastMortalities_wk51, linetype = "Mortalities Forecast for wk 51: LD_SD", group = 1)) + geom_line(size = 0.5, na.rm=TRUE, aes(y = FLD_ForecastMortality_wk51, linetype = "Mortalities Forecast for wk 51: Full lockdown", group = 1)) + scale_linetype_manual(values = c('dotted', 'dashed', 'solid')) + labs(color='Policy Id') + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + xlab("Week") + ylab("Count") + ggtitle("Effect: mortalities by policy") + theme(plot.title = element_text(hjust = 0.5)) + scale_colour_manual(values = colorSet2) + guides(color = guide_legend(override.aes = list(size = 1.5)))
	...
	if (isolate(input$tabs1) == "Hotspots") {
		plotDataImpMtly <<- isolate(plotDataMtlyHotspots)
	
	
		output$effectsHotSptMtly <<- renderPlotly({
			plotDataImpMtly()
		})
		
		outputOptions(output, "effectsHotSptMtly", suspendWhenHidden=FALSE)
	
	}
    
	
	#hotspots with other policies 
	
	plotDataHotspotsPolicyAddtn <- eventReactive(input$hotSptCasesProcessId, {
				
		par(mar=c(10,4.5,7,7))
		
		p <- ggplot(cvdData, aes(`Local Authority`)) + theme_bw() + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ActualCases_wk46, linetype = "Number of Actual Cases for wk 46: LD_SD", group = 1, color=colorSet2[1])) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastCases_wk51_Retail50Pct, linetype = "Cases Forecast for wk 51: LD_SD and 50% retail closing", group = 1, color=colorSet2[2])) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastCases_wk51_FoodAccom50Pct, linetype = "Cases Forecast for wk 51: LD_SD and 50% food & accom closing", group = 1, color=colorSet2[3])) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastCases_wk51_pub50Pct, linetype = "Cases Forecast for wk 51: LD_SD and 50% pubs closing", group = 1, color=colorSet2[4])) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastCases_wk51_quarantine100Pct, linetype = "Cases Forecast for wk 51: LD_SD and 100% travel quarantine", group = 1, color=colorSet2[5])) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastCases_wk51_SchoolOpen50Pct, linetype = "Cases Forecast for wk 51: LD_SD and 50% schools closing", group = 1, color=colorSet2[6])) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastCases_wk51_IntTravel50Pct, linetype = "Cases Forecast for wk 51: LD_SD and 50% international travel reduction", group = 1, color=colorSet2[7])) + geom_line(size = 0.5, na.rm=TRUE, aes(y = FLD_ForecastCases_wk51, linetype = "Cases Forecast for wk 51: Full lockdown", group = 1, color=colorSet2[8])) + labs(color=NULL) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + xlab("Week") + ylab("Count") + ggtitle("Effect: cases by policy") + theme(legend.margin=margin(10, 0, 50, 0), plot.title = element_text(hjust = 0.5)) + guides(color = guide_legend(override.aes = list(size = 1.5)))  
				...
				fig <- fig %>%
				  add_annotations( text="Policy Id", xref="paper", yref="paper",
								  ...
								  legendtitle=TRUE, showarrow=FALSE )  %>%
				layout(legend=list(yanchor="top", tracegroupgap=0.1, font=list(size = 8), x = 1.02, y = 0.85), xaxis=list(tickangle=270, title = "\n Location (Local Authority)", tickfont = list(size = '15')))
	...
	if (isolate(input$tabs1) == "Hotspots") {
		plotDataImpCasesPolicyAddtn <<- isolate(plotDataHotspotsPolicyAddtn)
	
	
		output$effectsHotSptPolicyAddtn <<- renderPlotly({
			plotDataImpCasesPolicyAddtn()
		})
		
		outputOptions(output, "effectsHotSptPolicyAddtn", suspendWhenHidden=FALSE)
 ...
 observeEvent(input$mapProcessId, { 
  ...
  #Week with latest actual cases 
  observeEvent(input$mapActualProcessId, { 
  ...
  if (isolate(input$tabs1) == "Map: Cases") {
		...
		if (!input$simulation) {
			...
			plotDataLeafletActual <- eventReactive(input$mapActualProcessId, {
				...
				# wkActualMax <- max(jointWeekNCases_Mltd_time[!is.na(jointWeekNCases_Mltd_time$daily), ]$week)
				...
				#maxActualWkValue <- jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$week == wkActualMax, "daily"]
								
				#filter by weeks  
				#slctnCmbnd <- slctnCmbnd[slctnCmbnd$week%in% input$checkWeek, ]  #input$checkWeek
				slctnActual <- jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$week %in% wkActualMax, ] 
				
				colnames(slctnActual)[which(names(slctnActual) == "lad19_nm")]  <- c("lad20_nm")
				
				if (length(unique(slctnActual$week)) > 1) {
					slctnActualGrp <- aggregate(daily~lad20_nm+lad20_cd+week, slctnActual, sum, na.rm = FALSE)
				} else {
					slctnActualGrp <- slctnActual[, c("lad20_nm", "lad20_cd", "week", "daily")]
				}
				
				# combine with local authority spatial data ----------------------------------------------
				
				# remove LA codes not in jointWeekNCases_Mltd_time 
				engWLAuthBndryLnSF_poly <- engWLAuthBndryLnSF_poly[engWLAuthBndryLnSF_poly$lad19cd %in% as.character(slctnActualGrp$lad20_cd),]

				# remove duplicate LA codes 
				engWLAuthBndryLnSF_polyUniq <- engWLAuthBndryLnSF_poly[!duplicated(engWLAuthBndryLnSF_poly$lad19cd),]

				# merge slctnActualGrp LA code with engWLAuthBndryLnSF_polyUniq LA code 
				slctnActualGrp <- merge(slctnActualGrp, engWLAuthBndryLnSF_polyUniq, by.x="lad20_cd",by.y="lad19cd")
				...
				slctnActualGrpSF <- sf::st_as_sf(slctnActualGrp)
				...
				slctnActualGrpSF_longLatCase <- st_transform(slctnActualGrpSF, "+init=epsg:4326")
				...
				mypal <- colorBin(palette = colorSet2, domain = slctnActualGrpSF_longLatCase$daily, bins = IntvlBins, pretty = FALSE, na.color = "#808080", alpha = FALSE, reverse = FALSE, right = FALSE)
				...
				leaflet() %>% 
						...
						setView(lat = 52.1936,, lng = -1, zoom = 6) %>%
						addPolygons(data = uk, weight = 1, smoothFactor = 0.2, fillOpacity = 0.1) %>%
						addPolygons(data = slctnActualGrpSF_longLatCase,
									stroke = FALSE, smoothFactor = 0.2, fillOpacity = 0.5,
									fillColor = ~mypal(slctnActualGrpSF_longLatCase$daily),
									popup = paste("Region: ", slctnActualGrpSF_longLatCase$lad20_nm, "<br>",
												  "Value: ", slctnActualGrpSF_longLatCase$daily, "<br>",
												  "Week: ", slctnActualGrpSF_longLatCase$week, "<br>"),
									highlight = highlightOptions(weight = 10,
											   color = "orange",
											   fillOpacity = 5,
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", labels=c("0 - 250", "250 - 500", "500 - 750", "750 - 1000", 
                       "1000 - 1250", "1250 - 1500+"),  pal = mypal, values = slctnActualGrpSF_longLatCase$daily,
								  title = "Number of cases",
								  opacity = 1)
			...
			plotDataLeafletImp <<- isolate(plotDataLeafletActual) %>% debounce(500)
			
			output$mapActual <<- renderLeaflet({
				plotDataLeafletImp()
			}) 
			
			outputOptions(output, "mapActual", suspendWhenHidden=FALSE)
  ...
  if (isolate(input$tabs1) == "Map: Mortality") {
		...
		if (!input$simulation) {
			...
			plotDataLeafletMtly <- eventReactive(input$mapMtlyProcessId, {
				...
				IntvlBins <-c(0, 5, 10, 15, 20, 25, 30)
				...
				leaflet() %>% 
						...
						addPolygons(data = slctnCmbndSpSF_longLatCase,
									...
									highlight = highlightOptions(weight = 10,
											   ...
											   fillOpacity = 5,
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", labels=c("0 - 5", "5 - 10", "10 - 15", "15 - 20", 
                       "20 - 25", "25 - 30+"),  pal = mypal, values = slctnCmbndSpSF_longLatCase$`Predicted Mortality`,
								  title = "Number of Mortality",
								  opacity = 1)
			...
			plotDataLeafletSimMtly <- eventReactive(input$mapMtlyProcessId, {
				...
				IntvlBins <-c(0, 50, 100, 150, 200, 250, 300)
				...
				leaflet() %>% 
						...
						addPolygons(data = PHE_centreGrpSpSF_longLatCase,
									...
									highlight = highlightOptions(weight = 10,
											   ...
											   fillOpacity = 5,
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", labels=c("0 - 50", "50 - 100", "100 - 150", "150 - 200", 
                       "200 - 250", "250 - 300+"), pal = mypal, values = PHE_centreGrpSpSF_longLatCase$`Simulated Predicted Mortality`,
								  title = "Number of Mortality",
								  opacity = 1)
				...
				leaflet() %>% 
						...
						addPolygons(data = slctnCmbndSpSF_longLatCase_sim,
									...
									highlight = highlightOptions(weight = 10,
											   ...
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", labels=c("0 - 5", "5 - 10", "10 - 15", "15 - 20", 
                       "20 - 25", "25 - 30+"), pal = mypal, values = slctnCmbndSpSF_longLatCase_sim$`Simulated Predicted Mortality`,
								  title = "Number of Mortality",
								  opacity = 1)
  ...
  #Week with latest actual mortality 
  observeEvent(input$mapActualMtlyProcessId, { 
  ...
  if (isolate(input$tabs1) == "Map: Mortality") {
		...
		if (!input$simulation) {
			...
			plotDataLeafletActualMtly <- eventReactive(input$mapActualMtlyProcessId, {
				...
				wkActualMax <- max(jointWeekNCases_Mltd_time[!is.na(jointWeekNCases_Mltd_time$dailyMtly), ]$week)
					
				#maxActualWkValue <- jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$week == wkActualMax, "dailyMtly"]
				...
				if (length(unique(slctnActual$week)) > 1) {
					slctnActualGrp <- aggregate(dailyMtly~lad20_nm+lad20_cd+week, slctnActual, sum, na.rm = FALSE)
				} else {
					slctnActualGrp <- slctnActual[, c("lad20_nm", "lad20_cd", "week", "dailyMtly")]
				}
				...
				mypal <- colorBin(palette = colorSet2, domain = slctnActualGrpSF_longLatCase$dailyMtly, bins = IntvlBins, pretty = FALSE, na.color = "#808080", alpha = FALSE, reverse = FALSE, right = FALSE)
				...
				leaflet() %>% 
						...
						addPolygons(data = slctnActualGrpSF_longLatCase,
									stroke = FALSE, smoothFactor = 0.2, fillOpacity = 0.5,
									fillColor = ~mypal(slctnActualGrpSF_longLatCase$dailyMtly),
									popup = paste("Region: ", slctnActualGrpSF_longLatCase$lad20_nm, "<br>",
												  "Value: ", slctnActualGrpSF_longLatCase$dailyMtly, "<br>",
												  "Week: ", slctnActualGrpSF_longLatCase$week, "<br>"),
									highlight = highlightOptions(weight = 10,
											   color = "orange",
											   fillOpacity = 5,
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", labels=c("0 - 5", "5 - 10", "10 - 15", "15 - 20", 
                       "20 - 25", "25 - 30+"),  pal = mypal, values = slctnActualGrpSF_longLatCase$dailyMtly,
			...
			plotDataLeafletImp <<- isolate(plotDataLeafletActualMtly) %>% debounce(500)
			
			output$mapActualMtly <<- renderLeaflet({
				plotDataLeafletImp()
			}) 
			
			outputOptions(output, "mapActualMtly", suspendWhenHidden=FALSE)
  ...
  observeEvent(input$modelProcessId, {

  if (isolate(input$tabs1) == "Model") { 
 
   
	plotDataVarImprtnc <- eventReactive(input$modelProcessId, {
		...
		par(mar=c(10,4.5,7,7))
				...
				varImpMatx <- t(as.matrix(modelMaster$trainable_weights[[1]]))

				colnames(varImpMatx) <- colnames(data_orig_Num)

				varImp <- colSums(varImpMatx)

				varImpDf <- as.data.frame(varImp)

				varImpDf <- cbind(rownames(varImpDf), varImpDf)

				rownames(varImpDf) <- NULL

				colnames(varImpDf) <- c("variable", "impValue")

				p <- ggplot(data = varImpDf, aes(x = variable, y = impValue)) + theme_bw() + geom_bar(stat = 'identity', position = 'dodge') + labs(color=NULL)  + xlab("Variable") + ylab("Relative Importance") + ggtitle(paste0("Relative Importance of variables")) + theme(plot.title = element_text(hjust = 0), plot.background = element_rect(fill = "#F5F5F5", colour = "#F5F5F5")) + guides(color = guide_legend(override.aes = list(size = 1.5))) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + theme(legend.title=element_blank())
				...
				fig <- fig %>%
				  add_annotations( text="Variable Id", xref="paper", yref="paper",
								  x=1.02, xanchor="left",
								  y=0.65, yanchor="bottom",    # Same y as legend below
								  legendtitle=TRUE, showarrow=FALSE )  %>%
	...
	if (isolate(input$tabs1) == "Model") {
		plotDataVarImp <<- isolate(plotDataVarImprtnc)
	
	
		output$histogrmVariableImprtnc <<- renderPlotly({
			plotDataVarImp()
		})
		
		outputOptions(output, "histogrmVariableImprtnc", suspendWhenHidden=FALSE)
  ...
  observeEvent(input$modelCasesProcessId, {

  if (isolate(input$tabs1) == "Model") { 
 
   
	plotDataVarCasesImprtnc <- eventReactive(input$modelCasesProcessId, {
				
		par(mar=c(10,4.5,7,7))
			
				#cases layer weight multiplication ===================================================================================

				#GRU kernel multiplied by GRU recurrent_kernel transposed at weights for input to hidden layer 1
				gruWeightsLayer1 <- as.matrix(abs(modelMaster$trainable_weights[[1]])) %*% t(as.matrix(abs(modelMaster$trainable_weights[[2]])))

				#GRU_2 kernel multiplied by GRU_2 recurrent kernel transposed at weights for hidden layer 1 to hidden layer 2 
				gru2WeightsLayer2Cases <- as.matrix(abs(modelMaster$trainable_weights[[4]])) %*% t(as.matrix(abs(modelMaster$trainable_weights[[5]])))

				#multiply weights from layer 1 with weights from layer 2
				unflattenWeightEstmt <- gruWeightsLayer1 %*% gru2WeightsLayer2Cases

				#multiply estimated weights by output for main_output to obtain overall estimated weight for cases part of model 
				modelCasesWeightEst <- unflattenWeightEstmt %*% as.matrix(abs(modelMaster$trainable_weights[[10]]))


				varImpMatx <- t(modelCasesWeightEst)

				colnames(varImpMatx) <- colnames(data_orig_Num)

				varImpDf <- as.data.frame(t(varImpMatx))
				...
				# zero-mean the y axis importance score to enhance the contrast between variables 
				varImpDf$impValue <- varImpDf$impValue - mean(varImpDf$impValue)
				

				p <- ggplot(data = varImpDf, aes(x = variable, y = impValue)) + theme_bw() + geom_bar(stat = 'identity', position = 'dodge') + labs(color=NULL)  + xlab("Variable") + ylab("Relative Importance of Variables for Cases") + ggtitle(paste0("Relative Importance of variables")) + theme(plot.title = element_text(hjust = 0), plot.background = element_rect(fill = "#F5F5F5", colour = "#F5F5F5")) + guides(color = guide_legend(override.aes = list(size = 1.5))) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + theme(legend.title=element_blank())
				...
				fig <- fig %>%
				  add_annotations( text="Variable Id", xref="paper", yref="paper",
								  ...
								  y=0.65, yanchor="bottom",    # Same y as legend below
								  legendtitle=TRUE, showarrow=FALSE )  %>%
				layout( legend=list(y=0.65, yanchor="top" ), xaxis=list(tickangle=270, title = "\n Variables", tickfont = list(size = '15')))
	...
	if (isolate(input$tabs1) == "Model") {
		plotDataVarCasesImp <<- isolate(plotDataVarCasesImprtnc)
	
	
		output$histogrmVariableCasesImprtnc <<- renderPlotly({
			plotDataVarCasesImp()
		})
		
		outputOptions(output, "histogrmVariableCasesImprtnc", suspendWhenHidden=FALSE)
  ...
  observeEvent(input$modelMtlyProcessId, {

  if (isolate(input$tabs1) == "Model") { 
 
   
	plotDataVarMtlyImprtnc <- eventReactive(input$modelMtlyProcessId, {
		...
		par(mar=c(10,4.5,7,7))
				...
				#GRU_1 kernel multiplied by GRU_1 recurrent kernel transposed at weights for hidden layer 1 to hidden layer 2 
				gru1WeightsLayer2Cases <- as.matrix(abs(modelMaster$trainable_weights[[7]])) %*% t(as.matrix(abs(modelMaster$trainable_weights[[8]])))

				#multiply weights from layer 1 with weights from layer 2
				unflattenWeightEstmt <- gruWeightsLayer1 %*% gru1WeightsLayer2Cases

				#multiply estimated weights by output for aux_output to obtain overall estimated weight for cases part of model 
				modelCasesWeightEst <- unflattenWeightEstmt %*% as.matrix(abs(modelMaster$trainable_weights[[12]]))
				...
				p <- ggplot(data = varImpDf, aes(x = variable, y = impValue)) + theme_bw() + geom_bar(stat = 'identity', position = 'dodge') + labs(color=NULL)  + xlab("Variable") + ylab("Relative Importance of Variables for Mortality") + ggtitle(paste0("Relative Importance of variables")) + theme(plot.title = element_text(hjust = 0), plot.background = element_rect(fill = "#F5F5F5", colour = "#F5F5F5")) + guides(color = guide_legend(override.aes = list(size = 1.5))) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + theme(legend.title=element_blank())
	...
	if (isolate(input$tabs1) == "Model") {
		plotDataVarMtlyImp <<- isolate(plotDataVarMtlyImprtnc)
	
	
		output$histogrmVariableMtlyImprtnc <<- renderPlotly({
			plotDataVarMtlyImp()
		})
		
		outputOptions(output, "histogrmVariableMtlyImprtnc", suspendWhenHidden=FALSE)
  ...
  observeEvent(input$modelCorrProcessId, {

  if (isolate(input$tabs1) == "Model") { 
 
   
	plotDataVarCorr <- eventReactive(input$modelCorrProcessId, {
		...
		par(mar=c(10,4.5,7,7))
			...
			#Correlation plot of mean of parameters against their relative importance =============

			medianVar <- apply(data_orig_Num, 2, median) 

			medianVarDf <- as.data.frame(medianVar)

			medianVarDf <- cbind(rownames(medianVarDf), medianVarDf)

			rownames(medianVarDf) <- NULL

			colnames(medianVarDf) <- c("variable", "medianValue")

			#merge mean with variable relative importance
			medianNVarImp <- merge(medianVarDf, varImpDf, by.x="variable",by.y="variable")	



			p <- ggplot(data = medianNVarImp, aes(x= impValue, y= 4*medianValue)) + geom_point()
			...
			dataWithoutAge <-data_orig_Num[,which(!(colnames(data_orig_Num) %like% "age_"))]

			M <- cor(dataWithoutAge)
	...
	if (isolate(input$tabs1) == "Model") {
		plotDataVarCorr <<- isolate(plotDataVarCorr)
	
	
		output$histogrmVariableCorr <<- renderPlot({
			plotDataVarCorr()
		})
