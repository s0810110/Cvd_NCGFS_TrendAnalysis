...
#
cvdData <- fread('data/mapResultsHighestRateCities.csv',  select=c('Local Authority', 'LLD_SD_ActualCases_wk46', 'LLD_SD_ActualMortalities_wk46', 'LLD_SD_ForecastCases_wk51', 'LLD_SD_ForecastMortalities_wk51', 'FLD_ForecastCases_wk51', 'FLD_ForecastMortality_wk51'), data.table=FALSE)
...
#observe( { 
  ...
  if (isolate(input$tabs1) == "Hotspots") { 
	...
	plotDataMtlyHotspots <- eventReactive(input$hotSptCasesProcessId, {
		...
		p <- ggplot(cvdData, aes(`Local Authority`)) + theme_bw() + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ActualMortalities_wk46, linetype = "Number of Actual mortalities for wk 46: Local lockdown with social distancing", group = 1)) + geom_line(size = 0.5, na.rm=TRUE, aes(y = LLD_SD_ForecastMortalities_wk51, linetype = "Mortalities Forecast for wk 51: Local lockdown with social distancing", group = 1)) + geom_line(size = 0.5, na.rm=TRUE, aes(y = FLD_ForecastMortality_wk51, linetype = "Mortalities Forecast for wk 51: Full lockdown", group = 1)) + scale_linetype_manual(values = c('dotted', 'dashed', 'solid')) + labs(color='Policy Id') + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + xlab("Week") + ylab("Count") + ggtitle("Effect: mortalities by policy") + theme(plot.title = element_text(hjust = 0.5)) + scale_colour_manual(values = colorSet2) + guides(color = guide_legend(override.aes = list(size = 1.5)))
