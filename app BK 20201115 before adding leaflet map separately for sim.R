                          ...
                          # tabPanel("General Metrics", br(),
                                   sidebarLayout(
                                     sidebarPanel(style = "overflow-y:scroll; max-height: 600px; position:relative;",
										  ...
										  hidden(div(style="display:inline-block; padding:5px;", dropdownButton(
											...
											, 
									   h5(strong("Other Criteria")), 
									   div(style="margin-top: 5px;",
									   materialSwitch(inputId = "simulation", label = "Configure Model", status = "success")), 
									   ...
									   h5(strong("Process Plot")),
									   hidden(div(id="TrendButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "trendProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Trend Model</span>"), width= 105, style='padding:4px; font-size:80%')) ), 
									   hidden(div(id="MapButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "mapProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Map Model</span>"), width= 100, style='padding:4px; font-size:80%')) ), br(),
									   hidden(div(id="ResetPrmtrButton", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "resetPrmtrButtonId", label =  shiny::HTML("<span style='font-size: 90%;'>Reset simulation variables</span>"), width= 100, style='padding:4px; font-size:80%')) ),br(),
								  ...
								  mainPanel( 
									 tabsetPanel(id="tabs1",
										tabPanel("General Metrics", br(),textOutput(outputId = "message") 				
								
										),
									    tabPanel("Trend", br(),
										   plotOutput(outputId = "plot_access_web", height = "600px"), br(), br(),
										   uiOutput("MtlyInstrctn"), br(),
										   div(id="TrendButtonMtly", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "trendProcessMtlyId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Mtly Model</span>"), width= 105, style='padding:4px; font-size:80%')),
...
#observe( { 
  ...
  } else if (isolate(input$tabs1) == "Map") { 
    show("MapButtonCases")
	hide("TrendButtonCases")
	show("ResetPrmtrButton")
  } else if (isolate(input$tabs1) == "Map: Density") { 
  ...
  output$sliderQrntnRestrctn <- renderUI({
			...
			if(flagResetVar$reset_var == TRUE) {
				...
				sliderInput(inputId = "travelQrntnRestrctn", label = "Inverse Travelling Quarantine Restrictions", min(jointWeekNCases_Mltd_time$QuarantineMeasures, na.rm = TRUE), max(jointWeekNCases_Mltd_time$QuarantineMeasures, na.rm = TRUE), value = unique(jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$wk_int == max(jointWeekNCases_Mltd_time$wk_int),]$QuarantineMeasures), round = 2, animate = TRUE)
  ...
  output$sliderIntrntlInflow <- renderUI({
			...
			if(flagResetVar$reset_var == TRUE | flagUpdtVarByLA$updateParamByLA == TRUE) {
			
				sliderInput(inputId = "intrntlInflow", label = "International Inflow", min(jointWeekNCases_Mltd_time$`International Migration Inflow`, na.rm = TRUE), max(jointWeekNCases_Mltd_time$`International Migration Inflow`, na.rm = TRUE), value =mean(unique(jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$wk_int == max(jointWeekNCases_Mltd_time$wk_int) & jointWeekNCases_Mltd_time$lad19_nm %in% input$check2,]$`International Migration Inflow`)), round = 2, animate = TRUE)
  ...
  output$sliderTransprtStrgSize <- renderUI({
			...
			if(flagResetVar$reset_var == TRUE | flagUpdtVarByLA$updateParamByLA == TRUE) {
				...
				sliderInput(inputId = "TransprtStrgSize", label = "Number of Transport and Storage services", min(jointWeekNCases_Mltd_time$Transport_Storage_inc_postal, na.rm = TRUE), max(jointWeekNCases_Mltd_time$Transport_Storage_inc_postal, na.rm = TRUE), value = mean(unique(jointWeekNCases_Mltd_time[jointWeekNCases_Mltd_time$wk_int == max(jointWeekNCases_Mltd_time$wk_int) & jointWeekNCases_Mltd_time$lad19_nm %in% input$check2,]$Transport_Storage_inc_postal)), round = 2, animate = TRUE)
 ...
 # if (input$plot1 == "Trend: expected and observed cases" && isolate(input$tabs1) == "Trend") {
  if (isolate(input$tabs1) == "Trend") { 
    # output$map <<- NULL
	# output$plot_access_web <<- NULL
   
	#output$plot_access_web <- eventReactive(flagPr$processButton, renderPlot({
	plotData <- eventReactive(input$trendProcessId, {
		...
		par(mar=c(10,4.5,7,7))
				...
				#filter by weeks  
				#subsetDt <- subsetDt[subsetDt$weekNumber %in% input$checkWeek, ]  #input$checkWeek
				...
				#randomLACode <- "E07000065"

				#randomLACode <- "E09000006"

				#randomLACode <- "E07000064"

				#randomLACode <- "E07000200"
				...
				#data_indiv <- subsetDt_indiv[ , 6:(ncol(subsetDt_indiv) - 1)]    #remove non-numeric columns 
				...
				samplesActual <- array(0, dim = c(length(subsetDt_indiv$week) - 1,
												2,
												dim(data_indiv)[[-1]]))  
				#targets <- array(0, dim = c(length(subsetDt$weekNumber) - 2))
				...
				for (i in (seq_along(1: (length(subsetDt_indiv$week) - 1))) ) {  # start from wk 5 to 41 (length 37) - 2
					...
					#indices <- seq(rows, rows + 2, length.out = dim(samples)[[2]]) #get consecutive 3 timestep values
					
					#indices <- seq(rows, rows + lookback - 1, length.out = dim(samples)[[2]]) #get consecutive 3 timestep values
				...
				#outputCasesUnscld <- prdctOutpt[,,1][,1] * stdTrain[["daily"]] + meanTrain[["daily"]]
				...
				outputUnscldWk <- cbind(prdctdWks, outputCasesUnscld)
				...
				#data_preJntPrdct <- merge(x = subsetDt, y = outputUnscldWk, by = c("weekNumber", "lad19_cd"), all = TRUE)
				...
				#data_preJntPrdct <- merge(x = subsetDt_indiv, y = outputUnscldWk, by = c("weekNumber", "lad19_nm"), all = TRUE)
				
				#data_preJntPrdct <- coalesce(subsetDt, outputUnscldWk)
				...
				#modify Growth_Rate using slider
				#if Growth_Rate is modified 
				
				# #modify variable here
				# grwthRIndx <- grep("Growth_Rate", colnames(data_indiv))
				
				# sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,grwthRIndx] <- scale(input$growthRate, center = meanTrain[["Growth_Rate"]], scale = stdTrain[["Growth_Rate"]])
				
				# #modify R number
				# rNumberIndx <- grep("R_number", colnames(data_indiv))
				
				# sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,rNumberIndx] <- scale(input$rNumber, center = meanTrain[["R_number"]], scale = stdTrain[["R_number"]])
				
				#modify spain travel 
				travelSpainIndx <- grep("TravelSpainEstimate", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,travelSpainIndx] <- scale(input$TravelSpain, center = meanTrain[["TravelSpainEstimate"]], scale = stdTrain[["TravelSpainEstimate"]])
				
				#lockdown
				lockDownIndx <- grep("LockdownScore", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,lockDownIndx] <- scale(input$LockDown, center = meanTrain[["LockdownScore"]], scale = stdTrain[["LockdownScore"]])
				
				#travelQrntnRestrctn
				travelQuarantnRstrctnIndx <- grep("QuarantineMeasures", colnames(data_indiv)) 
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,travelQuarantnRstrctnIndx] <- scale(input$travelQrntnRestrctn, center = meanTrain[["QuarantineMeasures"]], scale = stdTrain[["QuarantineMeasures"]])
				
				#internation inflow
				internatnlInflowIndx <- grep("International Migration Inflow", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,internatnlInflowIndx] <- scale(input$intrntlInflow, center = meanTrain[["International Migration Inflow"]], scale = stdTrain[["International Migration Inflow"]])
				
				#Pub size 
				pubSizeIndx <- grep("pub_count_2018", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,pubSizeIndx] <- scale(input$pubSize, center = meanTrain[["pub_count_2018"]], scale = stdTrain[["pub_count_2018"]])
				
				#retailShops
				pubSizeIndx <- grep("Retail", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,pubSizeIndx] <- scale(input$retailShops, center = meanTrain[["Retail"]], scale = stdTrain[["Retail"]])
				
				#open school
				schoolOpenIndx <- grep("School_Opening", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,schoolOpenIndx] <- scale(input$openSchool, center = meanTrain[["School_Opening"]], scale = stdTrain[["School_Opening"]])
				
				#Internal Migration Net flow
				InternalMigrationNetIndx <- grep("Internal Migration Net", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,InternalMigrationNetIndx] <- scale(input$internalMigNet, center = meanTrain[["Internal Migration Net"]], scale = stdTrain[["Internal Migration Net"]])
				
				#Number of Education establishment in 2020
				educatnEstblshmIndx <- grep("Education", colnames(data_indiv))
				
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,educatnEstblshmIndx] <- scale(input$eductnSize, center = meanTrain[["Education"]], scale = stdTrain[["Education"]])
				...
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,accmdNFoodSrvcIndx] <- scale(input$AccmNFoodSize, center = meanTrain[["Accommodation_food_services"]], scale = stdTrain[["Accommodation_food_services"]])
				...
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,accmdNFoodSrvcIndx] <- scale(input$TransprtStrgSize, center = meanTrain[["Transport_Storage_inc_postal"]], scale = stdTrain[["Transport_Storage_inc_postal"]])
				
				# #Overall imd deprivation rank 
				# imdRankIndx <- grep("IMD_Rank", colnames(data_indiv))
				
				# sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,imdRankIndx] <- scale(input$imdRank, center = meanTrain[["IMD_Rank"]], scale = stdTrain[["IMD_Rank"]])
				
				# #Living_Environment_Rank deprivation rank 
				# imdRankIndx <- grep("Living_Environment_Rank", colnames(data_indiv))
				
				# sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,imdRankIndx] <- scale(input$livingEnvRank, center = meanTrain[["Living_Environment_Rank"]], scale = stdTrain[["Living_Environment_Rank"]])
				
				# #Health_Deprivation_Disability_Rank deprivation rank 
				# healthDeprivatnRankIndx <- grep("Health_Deprivation_Disability_Rank", colnames(data_indiv))
				
				# sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,healthDeprivatnRankIndx] <- scale(input$healthDprvRank, center = meanTrain[["Health_Deprivation_Disability_Rank"]], scale = stdTrain[["Health_Deprivation_Disability_Rank"]])
				...
				sample_sim[dimnames(sample_sim)[[1]] %in% input$checkWeek,,artEntrnmntIndx] <- scale(input$artEntertmntRcrtn, center = meanTrain[["Arts_entertainment_recreation_other_services"]], scale = stdTrain[["Arts_entertainment_recreation_other_services"]])
				
				
				#do prediction on simulation data sample_sim
				
				prdctOutpt_sim <- predict(model, sample_sim, steps=NULL)
				# outputUnscld_sim <- output_sim * stdTrain[["cvdCaseCount"]] + meanTrain[["cvdCaseCount"]]
				# outputUnscld_sim <- as.data.frame(outputUnscld_sim, drop=FALSE)
				
				#outputCasesUnscld_Sim <- output_sim[,,1][,1] * stdTrain[["daily"]] + meanTrain[["daily"]]
				...
				#predicted weeks are six wks ahead of sample week in first column i.e. also of original data 
				minPrdctdWk <- parse_number(min(subsetDt_indiv$week)) + (delay + 1)
				maxPrdctdWk <- parse_number(max(subsetDt_indiv$week)) -1 + (delay + 1)  # -1 because last week is part of preceding index row in samples 
				
				prdctdWks <- c(sprintf("wk_%02d", minPrdctdWk: maxPrdctdWk))

				outputUnscldWk_sim <- cbind(prdctdWks, outputCasesUnscld_Sim )
				
				colnames(outputUnscldWk_sim)[which(names(outputUnscldWk_sim) == "prdctdWks")]  <- c("week")
									
				colnames(outputUnscldWk_sim)[which(names(outputUnscldWk_sim) == "outputCasesUnscld_Sim")]  <- c("Predicted Cases")
				
				colnames(subsetDt_indiv)[which(names(subsetDt_indiv) == "lad19_nm")]  <- c("lad20_nm")
				
				#outputUnscldWk$lad19_nm <- unique(subsetDt_indiv$lad19_nm)

				outputUnscldWk_sim$lad20_nm <- unique(subsetDt_indiv$lad20_nm)
				outputUnscldWk_sim$lad20_cd <- unique(subsetDt_indiv$lad20_cd)
				
				outputUnscldWk_sim$PHEC19CD <- unique(subsetDt_indiv$PHEC19CD)
				outputUnscldWk_sim$PHEC19NM <- unique(subsetDt_indiv$PHEC19NM)

				#data_preJntPrdct <- merge(x = subsetDt, y = outputUnscldWk_sim, by = c("weekNumber", "lad19_cd"), all = TRUE)
								
				data_preJntPrdct_sim <- merge(x = subsetDt_indiv, y = outputUnscldWk_sim, by = c("week", "lad20_nm", "PHEC19CD", "lad20_cd", "PHEC19NM"), all = TRUE)  #using outer join 
				
				#data_preJntPrdct <- coalesce(subsetDt, outputUnscldWk_sim)
				
				data_preJntPrdct_sim$cvdCaseCountUnscld <- data_preJntPrdct_sim$cvdCaseCount * stdTrain[["daily"]] + meanTrain[["daily"]] 				
				
				slctnCmbnd_sim <- rbind(data_preJntPrdct_sim,slctnCmbnd_sim)
				...
				#temp: remove duplicate weeks: 
				slctnCmbnd_sim <- slctnCmbnd_sim[!duplicated(slctnCmbnd_sim$week),]
				slctnCmbnd <- slctnCmbnd[!duplicated(slctnCmbnd$week),]
				

				# merge slctnCmbnd LA code with localAuthDataLatest_Week LA code 
				#slctnCmbndLatest <- merge(slctnCmbnd, localAuthDataLatest_Week, by.x=c("weekNumber", "lad19_cd"),by.y=c("week", "code"), all.x = TRUE)						
				# this is updated case count
				...
				if (length(unique(slctnCmbnd$weekNumber)) != 1 ) {
					...
					#g <- ggplot(slctnCmbnd, aes(weekNumber, group = as.factor(lad19_nm))) + theme_bw() + geom_line(size = 0.5, na.rm=TRUE, aes(y = `Predicted Cases`, linetype = "Predicted Case Count", colour = as.factor(lad19_nm))) + labs(color='Location Id') + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + xlab("Week") + ylab("Count") + ggtitle("Trend: number of actual and predicted cases from wk 5 to 43") + theme(plot.title = element_text(hjust = 0.5)) + scale_colour_manual(values = colorSet2) + guides(color = guide_legend(override.aes = list(size = 1.5)))
					
					if (input$simulation) {
					g <- g + geom_line(size = 1, na.rm=TRUE, aes(y = slctnCmbnd_sim$`Predicted Cases`, linetype = "Simulated Prediction Case Count", colour = as.factor(slctnCmbnd_sim$lad20_nm)))
					
					g
					
					#g + geom_line(size = 0.5, na.rm=TRUE, aes(y = slctnCmbndLatest$daily, linetype = "Latest Actual Cases data", colour = as.factor(slctnCmbndLatest$lad19_nm)))
					
					} else {
					
					g 
					
					#g + scale_linetype_manual(values = c('solid', 'dashed')) + geom_line(size = 0.5, na.rm=TRUE, aes(y = slctnCmbndLatest$daily, group = as.factor(slctnCmbndLatest$lad19_nm), linetype = "Latest Actual Cases data", colour = as.factor(slctnCmbndLatest$lad19_nm)))
				...
				#no actions
				slctnCmbnd_simSlctn <- slctnCmbnd_sim[,c("cvdCaseCountUnscld", "week", "Predicted Cases")]
			    ...
			    #slctnCmbndSlctn$cvdCaseCountUnscld  <- addNA(factor(slctnCmbndSlctn$cvdCaseCountUnscld ))
				...
				# slctnCmbndLatestDaily <- aggregate(daily~week, slctnCmbndLatest, sum, na.action = na.pass)	
				
				# slctnCmbndLatestDaily$one <- 1			
				...
				if (length(unique(casesByLauthDstrctWk$weekNumber)) != 1 ) {
					...
					#g <- ggplot(casesByLauthDstrctWk, aes(weekNumber)) + theme_bw() + geom_line(na.rm=TRUE, aes(y = `Predicted Cases`,  linetype = "Predicted Case Count", group = 1, colour = as.factor(one)), size = 0.5) + labs(color=NULL) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, margin=margin(5,0,7,0),face="bold")) + xlab("Wk (week number)") + ylab("Count") + ggtitle("Trend: number of actual and predicted cases from wk 5 to 43") + theme(plot.title = element_text(hjust = 0.5)) + scale_colour_manual(labels = c("All Selected Regions"), values = "darkorange") + guides(color = guide_legend(override.aes = list(size = 1.5)), linetype = guide_legend(override.aes = list(size = 0.5)) ) 
					
					if (input$simulation) {
					g <- g + geom_line(size = 1, aes(y = casesByLauthDstrctWk_Sim$`Predicted Cases`, group = 1, linetype = "Simulated Prediction Case Count", colour = as.factor(casesByLauthDstrctWk_Sim$one)), na.rm=TRUE)
					
					g 
					
					#g + geom_line(size = 0.5, na.rm=TRUE, aes(y = slctnCmbndLatest$daily, group = 1, linetype = "Latest Actual Cases data", colour = as.factor(one)))					
					
					} else {
	
					#g + scale_linetype_manual(values = c('solid', 'dashed')) + geom_line(size = 0.5, na.rm=TRUE, aes(y = slctnCmbndLatestDaily$daily, group = 1, linetype = "Latest Actual Cases data", colour = as.factor(slctnCmbndLatestDaily$one)))
					g 
  ...
  if (isolate(input$tabs1) == "Trend") { 
	...
	plotDataMtly <- eventReactive(input$trendProcessMtlyId, {
		...
		par(mar=c(10,4.5,7,7))
				...
				#outputMtlyUnscld <- prdctOutpt[,,1][,2] * stdTrain[["dailyMtly"]] + meanTrain[["dailyMtly"]]
				...
				#predicted weeks are delay + 1 wks ahead of sample week in first column i.e. also of original data 			
				minPrdctdWk <- parse_number(min(subsetDt_indiv$week)) + (delay + 1)
				maxPrdctdWk <- parse_number(max(subsetDt_indiv$week)) -1 + (delay + 1) 
				
				#add wk and 0s for single digits
				prdctdWks <- c(sprintf("wk_%02d", minPrdctdWk: maxPrdctdWk))
				# prdctdWks <- as.data.frame(prdctdWks)
				
				outputUnscldWk <- cbind(prdctdWks, outputMtlyUnscld)
				...
				#outputMtlyUnscld_Sim <- prdctOutpt_sim[,,1][,2] * stdTrain[["dailyMtly"]] + meanTrain[["dailyMtly"]]
				...
				outputUnscldWk_sim <- cbind(prdctdWks, outputMtlyUnscld_Sim )
				
				colnames(outputUnscldWk_sim)[which(names(outputUnscldWk_sim) == "prdctdWks")]  <- c("week")
									
				colnames(outputUnscldWk_sim)[which(names(outputUnscldWk_sim) == "outputMtlyUnscld_Sim")]  <- c("Predicted Mortality")
				...
				data_preJntPrdct_sim$cvdMtlyCountUnscld <- data_preJntPrdct_sim$cvdMtlyCount * stdTrain[["dailyMtly"]] + meanTrain[["dailyMtly"]] 				
				...
				if (length(unique(slctnCmbnd$weekNumber)) != 1 ) {
					...
					if (input$simulation) {
					g <- g + geom_line(size = 1, na.rm=TRUE, aes(y = slctnCmbnd_sim$`Predicted Mortality`, linetype = "Simulated Prediction Mortality Count", colour = as.factor(slctnCmbnd_sim$lad20_nm)))
				...
				#no actions
				slctnCmbnd_simSlctn <- slctnCmbnd_sim[,c("cvdMtlyCountUnscld", "week", "Predicted Mortality")]
			    ...
			    #slctnCmbndSlctn$cvdMtlyCountUnscld  <- addNA(factor(slctnCmbndSlctn$cvdMtlyCountUnscld ))
				...
				if (length(unique(casesByLauthDstrctWk$weekNumber)) != 1 ) {
					...
					if (input$simulation) {
					g <- g + geom_line(size = 1, aes(y = casesByLauthDstrctWk_Sim$`Predicted Mortality`, group = 1, linetype = "Simulated Prediction Mortality Count", colour = as.factor(casesByLauthDstrctWk_Sim$one)), na.rm=TRUE)
 ...
 observeEvent(input$mapProcessId, { 
  ...
  #if (input$plot1 == "Heat Map: Number of Cases" && isolate(input$tabs1) == "Map") {
  if (isolate(input$tabs1) == "Map") {
  # output$map <<- NULL
  # output$plot_access_web <<- NULL
  # plotDataLeaflet <<- NULL
  ...
  plotDataLeaflet <- eventReactive(input$mapProcessId, {
			...
			subsetDt <- jointWeekNCases_Mltd_timeSpSortedScaled
				...
				subsetDt_indiv_sim <- duplicate(subsetDt_indiv, shallow = FALSE)			

				wkColNames <- subsetDt_indiv_sim$week[1:(length(subsetDt_indiv_sim$week) - lookback)]  #lookback currently 1
				...
				output_sim <- predict(model, sample_sim, steps=NULL)
				# outputUnscld_sim <- output_sim * stdTrain[["cvdCaseCount"]] + meanTrain[["cvdCaseCount"]]
				# outputUnscld_sim <- as.data.frame(outputUnscld_sim, drop=FALSE)
				...
				outputCasesUnscld_Sim <- output_sim[[1]][,,1][,2] * stdTrain[["daily"]] + meanTrain[["daily"]]
				outputCasesUnscld_Sim <- as.data.frame(outputCasesUnscld_Sim, drop=FALSE)
				
				library(readr)
								
				#predicted weeks are six wks ahead of sample week in first column i.e. also of original data 
				minPrdctdWk <- parse_number(min(subsetDt_indiv_sim$week)) + (delay + 1)
				maxPrdctdWk <- parse_number(max(subsetDt_indiv_sim$week)) -1 + (delay + 1)  # -1 because last week is part of preceding index row in samples 
				...
				colnames(subsetDt_indiv_sim)[which(names(subsetDt_indiv_sim) == "lad19_nm")]  <- c("lad20_nm")
				
				#outputUnscldWk$lad19_nm <- unique(subsetDt_indiv_sim$lad19_nm)

				outputUnscldWk_sim$lad20_nm <- unique(subsetDt_indiv_sim$lad20_nm)
				outputUnscldWk_sim$lad20_cd <- unique(subsetDt_indiv_sim$lad20_cd)
				
				outputUnscldWk_sim$PHEC19CD <- unique(subsetDt_indiv_sim$PHEC19CD)
				outputUnscldWk_sim$PHEC19NM <- unique(subsetDt_indiv_sim$PHEC19NM)

				#data_preJntPrdct <- merge(x = subsetDt, y = outputUnscldWk_sim, by = c("weekNumber", "lad19_cd"), all = TRUE)
								
				data_preJntPrdct_sim <- merge(x = subsetDt_indiv_sim, y = outputUnscldWk_sim, by = c("week", "lad20_nm", "PHEC19CD", "lad20_cd", "PHEC19NM"), all = TRUE)  #using outer join 
				...
				if (!lAuthFlag$grpLAuth) {  #Default to use PHE centres 
				...
				#filter by weeks  
				#slctnCmbnd <- slctnCmbnd[slctnCmbnd$week %in% input$checkWeek, ]  #input$checkWeek
				slctnCmbnd <- slctnCmbnd[slctnCmbnd$week %in% max(weekList), ] 
				...
				slctnCmbnd_sim <- slctnCmbnd_sim[slctnCmbnd_sim$week %in% max(weekList), ] 
				
				slctnCmbndGrp <- aggregate(`Predicted Cases`~PHEC19CD+PHEC19NM, slctnCmbnd_sim, sum, na.rm = FALSE)
				...
				mypal <- colorBin(palette = colorSet2, domain = PHE_centreGrpSpSF_longLatCase$`Predicted Cases`, bins = 5, pretty = TRUE, na.color = "#808080", alpha = FALSE, reverse = FALSE, right = FALSE)
				...
				leaflet() %>% 
						...
						addPolygons(data = PHE_centreGrpSpSF_longLatCase,
									...
									highlight = highlightOptions(weight = 10,
											   ...
											   fillOpacity = 5,
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", pal = mypal, values = PHE_centreGrpSpSF_longLatCase$`Predicted Cases`,
				...
				if (length(unique(slctnCmbnd$week)) > 1) {
					slctnCmbndGrp <- aggregate(`Predicted Cases`~lad20_nm+lad20_cd, slctnCmbnd, sum, na.rm = FALSE)
				} else {
					slctnCmbndGrp <- slctnCmbnd[, c("lad20_nm", "lad20_cd", "Predicted Cases")]
				}
				...
				if (length(unique(slctnCmbnd_sim$week)) > 1) {
					slctnCmbndGrp <- aggregate(`Predicted Cases`~lad20_nm+lad20_cd, slctnCmbnd_sim, sum, na.rm = FALSE)
				} else {
					slctnCmbndGrp <- slctnCmbnd_sim[, c("lad20_nm", "lad20_cd", "Predicted Cases")]
				}
				...
				mypal <- colorBin(palette = colorSet2, domain = slctnCmbndSpSF_longLatCase$`Predicted Cases`, bins = 5, pretty = TRUE, na.color = "#808080", alpha = FALSE, reverse = FALSE, right = FALSE)
				...
				leaflet() %>% 
						...
						addPolygons(data = slctnCmbndSpSF_longLatCase,
									...
									highlight = highlightOptions(weight = 10,
											   ...
											   fillOpacity = 5,
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", pal = mypal, values = slctnCmbndSpSF_longLatCase$`Predicted Cases`,
								  title = "Number of cases",
								  opacity = 1)
	...
	if (isolate(input$tabs1) == "Map") {
		...
		if (!input$simulation) {
			plotDataLeafletImp <<- plotDataLeaflet
			...
			plotDataLeafletImpSim <<- plotDataLeaflet
  ...
  output$map <- renderLeaflet({
			...
			if (!nrow(subsetDt) == 0) {
				...
				if (!lAuthFlag$grpLAuth) {  #Default to use Local Authority regions
				...
				mypal <- colorBin(palette = colorSet2, domain = localOfficeGrpSpSF_longLat$popDensity, bins = 5, pretty = TRUE, na.color = "#808080", alpha = FALSE, reverse = TRUE, right = FALSE)
				...
				leaflet() %>% 
						...
						addPolygons(data = localOfficeGrpSpSF_longLat,
									...
									highlight = highlightOptions(weight = 10,
											   ...
											   fillOpacity = 5,
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", pal = mypal, values = localOfficeGrpSpSF_longLat$popDensity,
				...
				mypal <- colorBin(palette = colorSet2, domain = ccgGrpGrpSpSF_longLat$popDensity, bins = 5, pretty = TRUE, na.color = "#808080", alpha = FALSE, reverse = TRUE, right = FALSE)
				...
				leaflet() %>% 
						...
						addPolygons(data = ccgGrpGrpSpSF_longLat,
									...
									highlight = highlightOptions(weight = 10,
											   ...
											   fillOpacity = 5,
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", pal = mypal, values = ccgGrpGrpSpSF_longLat$popDensity,
								  labFormat = labelFormat(suffix = "µ"),
								  title = "Cases per million",
								  opacity = 1)
