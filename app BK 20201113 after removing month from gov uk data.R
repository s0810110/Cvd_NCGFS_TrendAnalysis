...
# smp_size <- floor(0.75 * length(uniqueLA))
...
generator <- function(data, lookback = 3, delay = 1, min_index = 1, max_index = 10,
  ...
  data_pre <- data_pre[!duplicated(data_pre$week),]
...
# train_gen <- generator(
  ...
  # delay = delay,
  # min_index = 1,
  # max_index = 10093,		#increase from 10 to 18 # changed from 26 to 36 for implementation rather than evaluation
  # shuffle = FALSE,
  # step = step, 
...
# history <- model %>% fit_generator(
  ...
  # callbacks = list(
  # callback_model_checkpoint('C:\\Installation WorkPCToLaptop\\Dashboard\\cardioDshBCvdDL\\model\\weights.{epoch:02d}-{val_loss:.4f}.hdf5', monitor = "val_loss", verbose = 0, save_best_only = TRUE, save_weights_only = FALSE, mode = "min", period = NULL, save_freq = "epoch"), callback_early_stopping(monitor = "val_loss", mode = "min", min_delta = 0.001, patience=200, restore_best_weights = TRUE)),
  # epochs = 500,
...
#model <- load_model_hdf5("data/weights.06-0.0301_2_206epoch_early stop.hdf5")
...
model <- load_model_hdf5("data/weights.02-0.1290_2_miniEpoch500_delay3.hdf5")
...
ui <- function(){
  ...
  navbarPage(title=div(img(src="data/logoSCTS.png")), selected = "", collapsible = TRUE, inverse = TRUE, theme = shinytheme("spacelab"),
             tabPanel("",
                      fluidPage(
                          ...
                          # tabPanel("General Metrics", br(),
                                   sidebarLayout(
                                     sidebarPanel(style = "overflow-y:scroll; max-height: 600px; position:relative;",
									   ...
									   hidden(div(id="MapButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "mapProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Map Model</span>"), width= 100, style='padding:4px; font-size:80%')) ), br(), br(),
									   ...
									   sliderInput(inputId = "imdRank", label = "Overall Deprivation Rank", min(jointWeekNCases_Mltd_time$IMD_Rank, na.rm = TRUE), max(jointWeekNCases_Mltd_time$IMD_Rank, na.rm = TRUE), value = median(jointWeekNCases_Mltd_time$IMD_Rank, na.rm = TRUE), round = 2, animate = TRUE),
									   sliderInput(inputId = "livingEnvRank", label = "Living Environment Deprivation Rank", min(jointWeekNCases_Mltd_time$Living_Environment_Rank, na.rm = TRUE), max(jointWeekNCases_Mltd_time$Living_Environment_Rank, na.rm = TRUE), value = median(jointWeekNCases_Mltd_time$Living_Environment_Rank, na.rm = TRUE), round = 2, animate = TRUE),
									   sliderInput(inputId = "healthDprvRank", label = "Health and disability Deprivation Rank", min(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Rank, na.rm = TRUE), max(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Rank, na.rm = TRUE), value = median(jointWeekNCases_Mltd_time$Health_Deprivation_Disability_Rank, na.rm = TRUE), round = 2, animate = TRUE),
...
#observe( { 
  ...
  if (input$plot1 == "Trend: expected and observed cases" && isolate(input$tabs1) == "Trend") { 
	...
	plotData <- eventReactive(input$trendProcessId, {
		...
		par(mar=c(10,4.5,7,7))
				...
				minPrdctdWk <- parse_number(min(subsetDt_indiv$week)) + 4
				maxPrdctdWk <- parse_number(max(subsetDt_indiv$week)) -1 + 4
 ...
 observeEvent(input$mapProcessId, { 
  ...
  } else if (input$plot1 == "Cases by Hospital") {
  ...
  } else if (input$plot1 == "Trend: expected and observed mortality") {
