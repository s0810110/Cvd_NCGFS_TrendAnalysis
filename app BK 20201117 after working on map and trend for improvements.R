                          ...
                          # tabPanel("General Metrics", br(),
                                   sidebarLayout(
                                     sidebarPanel(style = "overflow-y:scroll; max-height: 600px; position:relative;",
									   ...
									   materialSwitch(inputId = "nationalSetting", label = "National", status = "success"), 
								  ...
								  mainPanel( 
									 tabsetPanel(id="tabs1",
									    ...
									    tabPanel("Trend", br(),
										   hidden(div(id="TrendButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "trendProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Cases Model</span>"), width= 110, style='padding:4px; font-size:80%')) ), 
 ...
 observeEvent(input$mapProcessId, { 
  ...
  #output$map <- renderLeaflet({
		...
		if (!input$simulation) {
			...
			subsetDt <- isolate(jointWeekNCases_Mltd_timeSpSortedScaled)
			...
			plotDataLeafletImp <<- isolate(plotDataLeaflet) %>% debounce(5000)
			...
			subsetDt <- isolate(jointWeekNCases_Mltd_timeSpSortedScaled)
				...
				rm(data_preJntPrdct_sim)	
				rm(subsetDt_indiv_sim)	
				rm(outputUnscldWk_sim)	
				#rm(nhsLclOffcBndryLn)		
				rm(minPrdctdWk)	
				rm(maxPrdctdWk)	
				rm(prdctdWks)
				rm(outputCasesUnscld_Sim)
				rm(output_sim)
				rm(sample_sim)
				rm(wkColNames)
				rm(samplesActual)
				...
				slctnCmbnd_sim <- slctnCmbnd_sim[slctnCmbnd_sim$week %in% max(slctnCmbnd_sim$week), ] 
				
				slctnCmbndGrp <- aggregate(`Predicted Cases`~PHEC19CD+PHEC19NM+week, slctnCmbnd_sim, sum, na.rm = FALSE)
				...
				if (length(unique(slctnCmbnd_sim$week)) > 1) {
					slctnCmbndGrp_sim <- aggregate(`Predicted Cases`~lad20_nm+lad20_cd+week, slctnCmbnd_sim, sum, na.rm = FALSE)
				} else {
					slctnCmbndGrp_sim <- slctnCmbnd_sim[, c("lad20_nm", "lad20_cd", "week", "Predicted Cases")]
				}
				...
				mypal <- colorBin(palette = colorSet2, domain = slctnCmbndSpSF_longLatCase_sim$`Predicted Cases`, bins = IntvlBins, pretty = FALSE, na.color = "#808080", alpha = FALSE, reverse = FALSE, right = FALSE )
				...
				leaflet() %>% 
						...
						addPolygons(data = slctnCmbndSpSF_longLatCase_sim,
									stroke = FALSE, smoothFactor = 0.2, fillOpacity = 0.5,
									fillColor = ~mypal(slctnCmbndSpSF_longLatCase_sim$`Predicted Cases`),
									popup = paste("Region: ", slctnCmbndSpSF_longLatCase_sim$lad20_nm, "<br>",
												  "Value: ", slctnCmbndSpSF_longLatCase_sim$`Predicted Cases`, "<br>",
									...
									highlight = highlightOptions(weight = 10,
											   ...
											   bringToFront = TRUE)) %>%
						leaflet::addLegend(position = "bottomright", labels=c("0 - 250", "250 - 500", "500 - 750", "750 - 1000", 
                       "1000 - 1250", "1250 - 1500+"), pal = mypal, values = slctnCmbndSpSF_longLatCase_sim$`Predicted Cases`,
								  title = "Number of cases",
								  opacity = 1)
			...
			plotDataLeafletImpSim <<- isolate(plotDataLeafletSim) %>% debounce(5000)
