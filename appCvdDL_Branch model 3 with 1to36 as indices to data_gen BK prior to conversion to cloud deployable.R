...
# localAuthDataLatest_Week <-aggregate(. ~ week+code+name+month, localAuthDataLatest, sum)
...
# write.csv(x=localAuthDataLatest_Week_Policy, file="C:\\Installation WorkPCToLaptop\\Dashboard\\cardioDshBCvdDL\\data\\localAuthDataLatest_Week_Policy.csv")
...
# Model 2: delay <- 0 and max_index = 37,    ---------------------------------------------------------------
...
# train_gen <- generator(
  ...
  # delay = delay,
  # min_index = 1,
  # max_index = 37,		#increase from 10 to 18 # changed from 26 to 36 for implementation rather than evaluation
  # shuffle = TRUE,
  # step = step, 
  # batch_size = batch_size	#increase from 5 to 12 to increase sample size 
# )
                          ...
                          # tabPanel("General Metrics", br(),
                                   sidebarLayout(
                                     sidebarPanel(style = "overflow-y:scroll; max-height: 600px; position:relative;",
									   ...
									   hidden(div(id="TrendButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "trendProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Trend Model</span>"), width= 97, style='padding:4px; font-size:80%')) ), 
									   hidden(div(id="MapButtonCases", style="display:inline-block; margin-top: 5px;", actionButton(inputId = "mapProcessId", label =  shiny::HTML("<span style='font-size: 90%;'>Process Map Model</span>"), width= 97, style='padding:4px; font-size:80%')) ), br(), br(),
								  ...
								  mainPanel( 
									 tabsetPanel(id="tabs1",
									    ...
									    tabPanel("Trend", br(),
										   plotOutput(outputId = "plot_access_web", height = "600px")
									    ),
									    tabPanel("Map", br(),
  ...
  if (input$plot1 == "Trend: expected and observed cases" && isolate(input$tabs1) == "Trend") { 
	...
	plotData <- eventReactive(input$trendProcessId, {
		...
		par(mar=c(10,4.5,7,7))
				...
				prdctdWks <- c(sprintf("wk_%s", minPrdctdWk: maxPrdctdWk))
