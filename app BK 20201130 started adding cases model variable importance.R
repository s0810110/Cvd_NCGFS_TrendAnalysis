								  ...
								  mainPanel( 
									 tabsetPanel(id="tabs1",
									    ...
									    tabPanel("Map: Cases", br(),
										   ...
										   div(leafletOutput(outputId='mapSim', width = "80em", height = "80em")), br(), br()
...
#observe( { 
 ...
 observeEvent(input$mapProcessId, { 
  ...
  if (isolate(input$tabs1) == "Model") { 
	...
	plotDataVarImprtnc <- eventReactive(input$modelCasesProcessId, {
		...
		par(mar=c(10,4.5,7,7))
				...
				gruWeightsLayer1 <- as.matrix(modelMaster$trainable_weights[[1]]) %*% t(as.matrix(modelMaster$trainable_weights[[2]]))

				#GRU_2 kernel multiplied by GRU_2 recurrent kernel transposed at weights for hidden layer 1 to hidden layer 2 
				gru2WeightsLayer2Cases <- as.matrix(modelMaster$trainable_weights[[7]]) %*% t(as.matrix(modelMaster$trainable_weights[[8]]))
				...
				modelCasesWeightEst <- unflattenWeightEstmt %*% as.matrix(modelMaster$trainable_weights[[10]])

				t(modelCasesWeightEst)
