...
history <- model %>% fit_generator(
  ...
  callbacks = list(
  callback_model_checkpoint('model_trainingOnly/weights.{epoch:02d}-{val_loss:.5f}.hdf5', monitor = "val_loss", verbose = 0, save_best_only = TRUE, save_weights_only = FALSE, mode = "min", period = NULL, save_freq = "epoch"), callback_early_stopping(monitor = "val_loss", mode = "min", min_delta = 0.001, patience=300, restore_best_weights = TRUE)),
  epochs = 500,
...
model <- load_model_hdf5("model_trainingOnly/model5wksAhead/weights.05-0.17456_4_500epochsEarlyStop.hdf5")
